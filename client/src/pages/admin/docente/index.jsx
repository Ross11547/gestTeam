...
const siglaFromNombreCarrera = (nombre = "") => {
...
const Docentes = () => {
  ...
  const siglaCarrera = useMemo(
    () => carreraSel ? (carreraSel.sigla || siglaFromNombreCarrera(carreraSel.nombre)) : "",
    [carreraSel]
  );
  ...
  const previewCorreo = useMemo(() => {
    ...
    const local = joinDot("cbbe", nombres.replace(/\s+/g, ""), ap1, ap2);
  ...
  const handleUpdate = async (id, actualizado) => {
    try {
      const r = await authFetch(`${BASE}/api/docente/${id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const r = await authFetch(`${BASE}/api/docente/${id}`, { ...FETCH_OPTS, method: "DELETE" });
      if (!r.ok) {
        const txt = await readText(r);
  ...
  async function openEdit(row) {
    try {
      const { data } = await fetchDocenteById(row.id);
      setEditingId(row.id);
        ...
        <ModalContent size="large">
          ...
          <Form columns={2} onSubmit={onSubmit}>
            ...
            <FormGroup>
              ...
              <Input type="email" value={previewCorreo} readOnly title="cbbe.<nombres>.<primer_apellido>.<2 letras segundo apellido>@unifranz.edu.bo" />
            </FormGroup>
